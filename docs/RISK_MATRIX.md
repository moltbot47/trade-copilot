...
| R-12 | Technical | TradingView webhook is replayed by an attacker who guessed `bot_secret` (which is just `Bot.slug` — public). | 2 | 3 | **6** | Backend | Treat as bot-side authentication only — TradingView itself does not sign payloads. Mitigations: rate-limit `/webhooks/tradingview` (Wave 2); rotate slugs to opaque random strings (Wave 2); add HMAC-signed `?sig=` query param. |
...
|------|------|-------|--------|
| 1 | R-04 — auth router not mounted | 9 | **Open — block next deploy.** |
| 2 | R-01 — regulatory framing | 6 | Mitigated by ADR-0003 + LEGAL.md; recurring review. |
| 3 | R-12 — webhook replay | 6 | Slated for Wave 2 (HMAC). |
| 4 | R-14 — observability gap | 6 | Wave 1E covers this. |
| 5 | R-15 — backup unvalidated | 6 | Wave 1E follow-up. |
